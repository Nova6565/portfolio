# Portfolio Content — Source of Truth

## Positioning

**Mohamed Adel Mahmoud**  
**AI/ML Engineer · Applied AI Engineer**  
Cairo, Egypt

Suggested hero line:

> Building intelligent systems from models to real-world products.

Suggested short introduction:

> Computer Science graduate focused on AI and cloud engineering, with hands-on experience building machine-learning, computer-vision, RAG, backend-API, knowledge-graph, and cloud-deployed solutions.

About direction:

> I am a Computer Science graduate from the Arab Academy for Science, Technology & Maritime Transport (AASTMT), focused on building practical AI systems end to end. My work spans machine learning, Generative AI, NLP, computer vision, RAG, knowledge graphs, APIs, cloud services, and AI application integration. I care about turning models into useful, explainable products rather than treating AI as isolated experiments.

## Education

- Bachelor of Science in Computer Science
- Arab Academy for Science, Technology & Maritime Transport (AASTMT)
- Graduated: 2026
- GPA: 3.54
- Graduation project: PharmaSafe — A+

## Project 01 — PharmaSafe

**Title:** PharmaSafe  
**Subtitle:** AI-Powered Medication Safety Platform  
**Label:** A+ Graduation Project

**Summary:** An end-to-end medication-safety platform combining drug-drug interaction analysis, patient-aware drug substitution, prescription OCR, patient-specific medication modeling, explainable safety reporting, web/mobile interfaces, and multiple medical data sources.

**Technology:** Python, FastAPI, React, Flutter, Neo4j, DrugBank, DDInter, PubMedBERT, NLP, Groq LLMs, SHAP / Explainable AI.

### Core capabilities

1. **Drug-drug interaction engine:** A hybrid DDI system using DrugBank/Neo4j and DDInter data, with a PubMedBERT fallback for interaction analysis.
2. **Drug substitution:** Filters and ranks potentially safer medication alternatives against the patient's current medication profile and available interaction evidence for pharmacist review.
3. **Static digital twin:** Models the patient's current medication state, compares original and substituted medication regimens, and generates explainable safety reports.
4. **Prescription OCR:** Converts handwritten medication information into structured fields through YOLO11n → TrOCR → domain-aware pharmaceutical-name correction.

### Validated metrics

- DDInter records: 90,543
- PubMedBERT fallback accuracy: 88.14%
- PubMedBERT macro-F1: 85.63%
- YOLO stage mAP@50: 0.7094
- TrOCR exact match: 90.8%
- DrugBank exact match after correction: 73.2%
- Full-pipeline detections: 1,213

Repository: see `02_LINKS_AND_CONTACT.md`.

## Project 02 — Intelligent Malware Analysis Platform

Cloud-based malware-risk analysis combining static machine-learning analysis, dynamic sandbox behavior, and VirusTotal hash verification.

- Dataset: EMBER2024
- Model: XGBoost
- AUC: 0.99
- Accuracy: 0.97
- Cloud: Huawei Cloud
- Services: ECS, OBS, VPC, Cloud Eye
- Public repository: not supplied; omit the GitHub button

## Project 03 — RAG Document Q&A Assistant

Source-grounded document question-answering system for PDF, DOCX, and TXT files.

- LangChain
- FAISS
- embeddings
- Groq LLMs
- Gradio
- FastAPI
- Focus: ingestion, retrieval, semantic search, grounding, and answer generation

Repository: see `02_LINKS_AND_CONTACT.md`.

## Project 04 — Brain Tumor Detection System

MRI-based brain-tumor classifier.

- PyTorch
- ResNet-18
- Approximately 7,000 MRI images
- 97% accuracy
- Best Project — Introduction to Artificial Intelligence

Repository: see `02_LINKS_AND_CONTACT.md`.

## Project 05 — Disaster Damage Assessment from Satellite Images

AI pipeline assessing building damage after disasters from pre/post satellite imagery.

- PyTorch
- Swin Transformer-Tiny
- SAM2
- computer vision, segmentation, and classification
- Accuracy: 78%
- Macro-F1: 76%

Repository: see `02_LINKS_AND_CONTACT.md`.

## Experience and programs

### Backend Intern — Connect Digital Solutions

Aug 2025 – Nov 2025

Developed Java Spring REST APIs and worked with professional API-oriented development workflows.

### Data Science Program — ALX Africa

Mar 2025 – Apr 2026

Completed analytical workflows, portfolio projects, and structured problem-solving practice.

### Data Science Track — DEPI

Oct 2024 – May 2025

Applied data analysis, machine learning, visualization, and practical project delivery.

## Skills

### AI / Machine Learning

Python, PyTorch, TensorFlow, Keras, scikit-learn, CNNs, transfer learning, computer vision, transformers, and model evaluation.

### Generative AI / RAG

LLMs, LangChain, FAISS, embeddings, prompt engineering, Groq LLMs, and source-grounded question answering.

### Backend / Software

Java, Spring Boot, FastAPI, and REST APIs.

### Frontend / Mobile

React, Flutter, HTML, and CSS.

### Data / Knowledge

Neo4j, SQL/data concepts, and knowledge graphs.

### Cloud

Huawei Cloud, ECS, OBS, VPC, Cloud Eye, and Oracle Cloud Infrastructure.

### Tools

Git, GitHub, Gradio, and Streamlit.

## Certifications and training

### Images included in this handoff

- ITI NVIDIA DLI Summer Training Program — Generative AI Beginner Level, 35 hours
- NVIDIA — Building LLM Applications with Prompt Engineering
- Huawei Cloud — HCCDA-AI
- Huawei Cloud — HCCDA-Tech Essentials
- Oracle Cloud Infrastructure Certified Foundations Associate
- Oracle Cloud Infrastructure Certified Architect Associate
- Oracle Agentic AI Certified Foundations Associate
- Digital Egypt Pioneers Program — Data Scientist
- ALX Data Science

### Documented credentials without an image in this handoff

- IBM AI Engineering Professional Certificate
- IBM Generative AI Engineering with LLMs Specialization
- NVIDIA — Building RAG Agents with LLMs
- Generative AI Advanced Level, 100 hours (show only if the final supporting evidence is added)

Text-only credential cards are acceptable when an image is absent. Never invent a preview.

## Achievements

- PharmaSafe — A+ Graduation Project
- Huawei Developer Competition Northern Africa 2025 — Top 50 among 389 teams and 1,400+ developers
- ECPC 2024 — Finalist
- Four academic course project awards
- Best Project — Introduction to Artificial Intelligence (Brain Tumor Detection)

Additional awards may exist, but do not invent undocumented award titles.

## Public contact

Use the exact values in `02_LINKS_AND_CONTACT.md`. Never show a phone number.

